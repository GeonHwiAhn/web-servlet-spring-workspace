	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		/*
		 ...
		 HttpServletRequest
		  ...
		  - 요청시 전달된 데이터, 요청한 클라이언트 정보, 요청을 융연하게 처리하기 위한 객체 등을 제공
		 ...
		 HttpServletResponse
		  ...
		  --> 제출된 이름과 나이 = 전달 매개변수(Parameter) == 전달된 값 보여줄 것
		  
		  --> 엉에 담긴 특정 파라미터 가져오기 name
		  ---> String HttpServletRequest.getParameter("name속성값");
		 * */
		// <input type="text" name = "inputName"> 에 작성되어 제출된 값 얻어오기
		String inputName = request.getParameter("inputName");
		//		--> 고객들의 작성된 값을 getParameter에서 name에 지정한 별칭으로 가져오기
